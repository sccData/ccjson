...
namespace json {
    ...
    JParser::JParser(std::shared_ptr<JValue>& val, const std::string& content)
            :val_(val), cur_(content.c_str())
    {
        // val_.set_type(json::LEPT_NULL);
        val_ = std::make_shared<JValue>();
        parse_whitespace();
        parse_value();
        parse_whitespace();
        if(*cur_ != '\0') {
    ...
    void JParser::parse_value() {
        switch(*cur_) {
            ...
            case 't': parse_literal("true", json::LEPT_BOOL);   return;
            case 'f': parse_literal("false", json::LEPT_BOOL); return;
    ...
    void JParser::parse_object() {
        expect(cur_, '{');
        parse_whitespace();
        std::unordered_map<std::string, std::shared_ptr<JValue>> tmp;
        std::string key;
        if(*cur_ == '}') {
            ++cur_;
            // val_.set_object(tmp);
